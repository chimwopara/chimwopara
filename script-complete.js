...
// === END CRITICAL FUNCTIONS ===
  ...
  { id: 'polar',       name: 'Polar Lights',       start: '#ecfeff', end: '#cffafe', accent: '#155e75', textColor: 'dark' },  // very light → deep cyan accent
        ...
        let linesRemaining = parseInt(localStorage.getItem('linesRemaining')) || 250;
        let maxLines = 250;
...
// Deduct lines after generating a challenge
function deductLines(count) {
    linesRemaining = Math.max(0, linesRemaining - count);
    localStorage.setItem('linesRemaining', linesRemaining.toString());
    updateLinesDisplay();
    
    // Show warning if running low
    if (linesRemaining <= 0) {
        showAlert("You've used all your lines. Upgrade to continue!", true, 5000);
    } else if (linesRemaining <= 20) {
        showAlert(`Only ${linesRemaining} lines remaining`, false, 3000);
    }
}

// Make deductLines globally accessible
window.deductLines = deductLines;
...
// Auto-apply pending theme if exists
...
if (savedUnlockedThemes) {
    ...
    // Save the desired theme before opening upgrade modal
    localStorage.setItem('pendingTheme', theme.id);
...
// Export sign-in functions globally
...
function updatePremiumModalUI() {
    ...
    updatePremiumModalUI();
        ...
        // Global exports for modal functions (accessible from onclick handlers)
        window.showThemeStoreModal = showThemeStoreModal;
        window.closeThemeStoreModal = closeThemeStoreModal;
        ...
        // Global exports for university modal
        ...
        function renderUniversityUI() {
            ...
            Object.keys(schoolData).forEach(key => {
                ...
                // Interaction removed - Informational only
    ...
    // --- NEW: Inject Profile Button ---
    let profileBtn = document.getElementById('portalProfileBtn');
    if (!profileBtn) {
        profileBtn = document.createElement('div');
        profileBtn.id = 'portalProfileBtn';
        profileBtn.className = 'portal-profile-btn';
        profileBtn.onclick = openUniSettings;
        portal.appendChild(profileBtn);
    }
    
    // Set Image (Google Profile or Default)
    const picSrc = userProfile?.picture || 'https://lh3.googleusercontent.com/a/default-user=s96-c';
    profileBtn.innerHTML = `<img src="${picSrc}" alt="Profile">`;
    // ----------------------------------
    ...
    loadWeekContent(currentWeek);
        ...
        function checkSelections() {
            ...
            // Code mode logic - only require input (language validation happens on submit)
            // This allows the button to be clickable, then we prompt for language if missing
            generateBtn.disabled = !hasInput;
        }
...
if (window.innerWidth <= 768) {
    document.addEventListener('click', (e) => {
        ...
        window.toggleSidebar = () => {
            ...
            // Add/remove expanded class to main content for centering
            if (sidebar.classList.contains('hidden')) {
                if (mainContent) mainContent.classList.add('sidebar-hidden');
                if (creatorPanel) creatorPanel.classList.add('centered');
            } else {
                if (mainContent) mainContent.classList.remove('sidebar-hidden');
        ...
        function loadChallengeStore() {
            // Load from new pnlStoreItems (auto-added challenges)
            ...
            storeItems.forEach((challenge, index) => {
                const card = document.createElement('div');
                ...
                meta.style.cssText = 'display: flex; gap: 12px; font-size: 0.85rem; color: var(--text-secondary); flex-wrap: wrap;';
                meta.innerHTML = `<span>${challenge.language || 'General'}</span><span>•</span><span>${challenge.difficulty || 'Intermediate'}</span><span>•</span><span>${challenge.stepsCount || 0} steps</span><span>•</span><span>${challenge.plays || 0} plays</span>`;
                
                const date = document.createElement('div');
                date.style.cssText = 'font-size: 0.75rem; color: var(--text-tertiary);';
                const createDate = new Date(challenge.createdAt); 
                date.textContent = `Created ${createDate.toLocaleDateString()}`;
                
                const btnContainer = document.createElement('div');
                btnContainer.style.cssText = 'display: flex; gap: 8px; margin-top: auto;';
                
                const playBtn = document.createElement('button');
                playBtn.className = 'minimal-btn'; 
                playBtn.style.cssText = 'flex: 1; justify-content: center; background: var(--accent-primary); color: white;';
                playBtn.textContent = 'Play';
                playBtn.onclick = (e) => { 
                    e.stopPropagation(); 
                    closeStoreModal();
                    ChallengeHandler.loadFromShareId(challenge.shareId); 
                };
                
                const shareBtn = document.createElement('button');
                shareBtn.className = 'minimal-btn';
                shareBtn.style.cssText = 'padding: 8px 12px;';
                shareBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>';
                shareBtn.title = 'Copy share link';
                shareBtn.onclick = (e) => { 
                    e.stopPropagation(); 
                    const shareUrl = window.location.origin + window.location.pathname + '?pnl=' + challenge.shareId;
                    navigator.clipboard.writeText(shareUrl).then(() => { 
                        showAlert('Link copied!', false, 2000); 
                    }).catch(() => { 
        ...
        // Make loadChallengeStore globally accessible
        window.loadChallengeStore = loadChallengeStore;
        ...
        window.shareCurrentChallenge = function() {
            // Use ChallengeHandler's current share ID if available
            if (window.ChallengeHandler && window.ChallengeHandler.currentShareId) {
                const shareUrl = window.location.origin + window.location.pathname + '?pnl=' + window.ChallengeHandler.currentShareId;
                navigator.clipboard.writeText(shareUrl).then(() => { 
                    showAlert('Link copied to clipboard!', false, 3000); 
                }).catch(() => { 
                    prompt('Copy this link:', shareUrl); 
                });
                return;
            }
            showAlert('No challenge to share', true, 2000);
        };
...
function handleLogout() {
    ...
    // --- NEW: Clear University Data ---
    localStorage.removeItem('isEnrolled');
    localStorage.removeItem('enrolledFaculty');
    isEnrolled = false;
    enrolledFaculty = null;
    document.getElementById('studentPortal').classList.remove('active'); // Close portal if open
    // ----------------------------------
...
function triggerSupportEmail() {
    window.location.href = "mailto:support@chimwopara.com";
        ...
        // Global exports for structured modal
        window.showStructuredModal = showStructuredModal;
        window.closeStructuredModal = closeStructuredModal;
...
// --- UNIVERSITY SETTINGS LOGIC ---

function openUniSettings() {
    const modal = document.getElementById('uniSettingsModal');
    
    // Generate Random Student ID if not exists
...
function closeUniSettings() {
    document.getElementById('uniSettingsModal').classList.remove('active');
}

function handleUniDropout() {
    if (confirm("Are you sure you want to drop out? This will remove your enrollment status.")) {
        localStorage.removeItem('isEnrolled');
        localStorage.removeItem('enrolledFaculty');
        isEnrolled = false;
        enrolledFaculty = null;
